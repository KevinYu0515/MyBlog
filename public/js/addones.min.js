  ...
  $(window).scroll(function () {
    ...
    $('[class="tag"]').each((e, l) => {
      let a = l.innerText.toLowerCase(),
        [t, o, s, r, _, n] = labels[a],
        c = (0.2126 * t + 0.7152 * o + 0.0722 * s) / 255,
        i = Math.max(0, Math.min(1 / (0.453 - c), 1)),
        h = Math.max(0, Math.min((c - 0.96) * 100, 1));
      (l.style.color = `hsl(0deg, 0%, calc(${i} * 100%))`),
        (l.style.background = `rgb(${t}, ${o}, ${s})`),
        (l.style[
          "border-color"
        ] = `hsla(${r}, calc(${_} * 1%), calc(${n} - 25) * 1%), ${h})`),
